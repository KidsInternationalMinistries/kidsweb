 ...
 var sessionmode = "payment"
  ...
  try {
    var arrPriceId = []
      ...
      const name = decodeURI(req.query.productName);
      const desc = decodeURI(req.query.productDesc);
      const image = req.query.productImage;
      const customText = "custom text"
                   
      if (recurring) sessionmode = "subscription"
      ...
      /// Create product if it does not exist
        try {
          ...
          await stripe.products.update(productId, { name: name,  description:desc, images: [image],});
        } 
        catch (error)
        {
        // Product does not exist, create it
        if (error.statusCode === 404) {
          const createdProduct = await stripe.products.create({id: productId, name: name,  description:desc, images: [image]})
    ...
    var stripeprice = Math.trunc(price * 100)
    var bFound = false;
    var prices = [];
    var has_more = true;
    while (has_more) {
    ...
    {
        if ( 
            (prices[i].currency == currency.toLowerCase()) && 
            (stripeprice == prices[i].unit_amount)  
            
        )
        { arrPriceId.push( {price: prices[i].id,quantity: 1} )
          bFound = true;
          break;
        }
    }
    ...
    if (!bFound) {
      ...
      if (recurring) create_priece.recurring = {interval: "month"} 
 
      var oPrice = await stripe.prices.create(create_priece);
      
      arrPriceId.push( {price: oPrice.id,quantity: 1})
      arrPriceId.push( {price: oPrice.id,quantity: 3})
      arrPriceId.push( {price: oPrice.id,quantity: 5})
    }
